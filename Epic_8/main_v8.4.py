...
###############################
          self.databaseCursor.execute("SELECT * FROM APPLICATIONS LEFT JOIN JOB_DATA ON APPLICATIONS.JOB_ID = JOB_DATA.JOB_ID WHERE JOB_DATA.JOB_ID IS NULL")# AND USER_ID = ?", (loginUsername))
          message = self.databaseCursor.fetchone()
          if message is None:
            messagebox.showwarning("Notification", "A job that you applied for has been deleted")

          self.databaseCursor.execute("SELECT * from JOB_DATA NATURAL JOIN USER_DATA WHERE CREATION_TIME>LAST_SEEN")
          message = self.databaseCursor.fetchone()
          print(message)
          if message is None:
