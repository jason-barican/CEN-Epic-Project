    ...
    for i in range(0, len(accounts)):
      if loginUsername in accounts[i]:
        if loginPassword in accounts[i]:
          ...
          if profileInfo == None:
            messagebox.showwarning("Profile not found", "Don't forget to create a profile")

          self.databaseCursor.execute("SELECT * from MESSAGES WHERE RECEIVER = ?", (loginUsername,))
          message = self.databaseCursor.fetchone()
          if message is not None:
            messagebox.showwarning(loginUsername, "You have messages waiting for you")

          self.databaseCursor.execute("SELECT * FROM MESSAGES WHERE RECEIVER = ? AND NEW_USER = 1", (loginUsername,))
          sender = self.databaseCursor.fetchone()

          if sender is not None:
              sender = sender[0]
              cursor = self.databaseCursor.execute("SELECT FIRSTNAME, LASTNAME FROM USER_DATA WHERE USERNAME = ?", (sender,))
              rows = cursor.fetchall()
              if rows:
                  FIRSTNAME = rows[0][0]
                  LASTNAME = rows[0][1]
                  messagebox.showwarning("New user", FIRSTNAME + " " + LASTNAME + " has joined InCollege")
              self.databaseCursor.execute("DELETE FROM MESSAGES WHERE RECEIVER = ? AND SENDER = ? AND NEW_USER = 1", (loginUsername, sender))
...
class SignUpWindow(tk.Frame):
  ...
  def checkIfValid(self):
    ...
    if usernameValid is True and passwordValid is True and fullNameValid is True and tierValid is True:
      ...
      self.cursor.execute(query, (self.newUsername,))
      rows = self.cursor.fetchall() 
      for row in rows:
          self.cursor.execute("INSERT INTO MESSAGES (RECEIVER, SENDER, NEW_USER) VALUES (?, ?, ?)", (row[0], self.newUsername, 1))
      self.conn.commit()
