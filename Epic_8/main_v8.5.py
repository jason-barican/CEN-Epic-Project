...
class LoginWindow(tk.Frame):
  ...
  def login(self):
    global loginUsername
    global selectedProfileUsername
    global lastSeenTime
    loginUsername = self.usernameEntry.get()
    selectedProfileUsername = loginUsername
    ...
    for i in range(0, len(accounts)):
      if loginUsername in accounts[i]:
        if loginPassword in accounts[i]:
          ...
          self.databaseCursor.execute("SELECT LAST_SEEN FROM USER_DATA WHERE USERNAME = ?",(loginUsername,))
          lastSeenTime = float(self.databaseCursor.fetchone()[0])

          self.databaseCursor.execute("SELECT FIRSTNAME, LASTNAME, USERNAME FROM USER_DATA WHERE CAST(CREATION_TIME AS decimal) > ?",(lastSeenTime,))
          newUsers = self.databaseCursor.fetchall()
          print(newUsers)
          if len(newUsers) > 0:
            userList = ""
            for user in newUsers:
              userList += "Name: " + user[0] + " " + user[1] + ", Username: " + user[2] + "\n"

            messagebox.showwarning("Notification", "New users have joined InCollege:\n" + userList)

          currentTime = time.time()
          self.databaseCursor.execute("UPDATE USER_DATA SET LAST_SEEN = ? WHERE USERNAME = ?", (currentTime,loginUsername))
          self.database.commit()

          self.databaseCursor.execute("SELECT * from PROFILE_DATA WHERE USERNAME = ?", (loginUsername,))
          profileInfo = self.databaseCursor.fetchone()
          if profileInfo == None:
            messagebox.showwarning("Notification", "It appears that you have not created a profile yet. Don't forget to create a profile!")

          self.databaseCursor.execute("SELECT * from MESSAGES WHERE RECEIVER = ?", (loginUsername,))
          message = self.databaseCursor.fetchone()
          if message is not None:
            messagebox.showwarning("Notification", "You have messages waiting for you!")
###############################
          lastWeekTime = time.time() - 604800
          user_id = self.databaseCursor.execute("SELECT USER_ID FROM USER_DATA WHERE USERNAME = ?", (loginUsername,)).fetchone()[0]
          self.databaseCursor.execute("SELECT * FROM APPLICATIONS WHERE CAST(CREATION_TIME AS decimal) > ? AND USER_ID = ?",(lastWeekTime, user_id))
          last7daysApp = self.databaseCursor.fetchone()
          if last7daysApp is None:
            messagebox.showwarning("Notification", "Remember – you're going to want to have a job when you graduate. Make sure that you start to apply for jobs today!")
  ...
  def checkIfValid(self):
    ...
    if len(self.newPassword) >= 8 and len(self.newPassword) <= 12 and self.newPassword.lower() != self.newPassword:
      for char in self.newPassword:
        if char.isdigit():
          hasDigit = True
    if hasDigit is False or self.newPassword.isalnum():
    ...
    if usernameValid is True and passwordValid is True and fullNameValid is True and tierValid is True:
      data_insert_query = ('''INSERT INTO USER_DATA(
                          USER_ID, USERNAME, PASSWORD, FIRSTNAME, LASTNAME, EMAIL_PREF, SMS_PREF, TA_PREF, LANG_PREF, TIER, CREATION_TIME, LAST_SEEN) VALUES
                          (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                          ''')
      count += 1
      ...
      data_insert_tuple = (count, self.newUsername, self.newPassword, 
                           ...
                           self.clicked.get(), currentTime, currentTime)
...
class ApplicationWindow(tk.Frame):
  def __init__(self, parent, controller):
    ...
    DisplayProfileButton = tk.Button(self, text = "Display Profile", command = self.display_profile)
...
# Create the second dropdown menu with the options "Copyright Notice", "About", "Accessibility", "User Agreement", "Privacy Policy", "Cookie Policy", "Copyright Policy", "Brand Policy", "Guest Controls", and "Languages"
  ...
  def display_profile(self):
    ...
    global selectedProfileUsername
    selectedProfileUsername = loginUsername
    self.controller.show_frame("DisplayProfileFrame")
...
#########################################
    ...
    self.cursor = self.conn.cursor()
    # Create a Button
    button = Button(self, text="Notifications", command=self.app_count, height=5, width=10)
...
#######################################
  ...
  def app_count(self):
    global loginUsername
    user_id = self.cursor.execute("SELECT USER_ID FROM USER_DATA WHERE USERNAME = ?", (loginUsername,)).fetchone()[0]
    app_count = 0
    app_count = self.cursor.execute("SELECT COUNT(*) FROM APPLICATIONS WHERE USER_ID = ?",(user_id,)).fetchone()[0]
    messagebox.showinfo("Application Count", "You have applied for " + str(app_count) + " jobs.")
  ...
  def on_show_frame(self, parent):
    ...
    self.cursor.execute("SELECT LAST_SEEN FROM USER_DATA WHERE USERNAME = ?",(loginUsername,))
    self.cursor.execute("SELECT TITLE from JOB_DATA WHERE CAST(CREATION_TIME AS decimal) > ?",(lastSeenTime,))
    newJobs = self.cursor.fetchall()
    print(newJobs)
    if len(newJobs) > 0:
      jobList = ""
      for job in newJobs:
      ...
      messagebox.showwarning("Notification", "A new job has been posted:\n" + jobList)
  ...
  def post_job(self):
      ...
      if count >= 10:	
        ...
        data_insert_query = ('''INSERT INTO JOB_DATA(	
                            JOB_ID, TITLE, DESCRIPTION, EMPLOYER, LOCATION, SALARY, POSTED_BY, CREATION_TIME) VALUES	
                            (?, ?, ?, ?, ?, ?, ?, ?)	
                            ''')	
        data_insert_tuple = (job_id, title, description, employer, location, salary, user_id, currentTime)	
...
class JobApplicationFrame(tk.Frame):
  ...
  def apply_job(self):
      ...
      self.cursor.execute('INSERT OR IGNORE INTO APPLICATIONS (JOB_ID, USER_ID, TITLE, EMPLOYER, DESCRIPTION, GRADUATION_DATE, START_DATE, WHY, CREATION_TIME) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
                            (job_id, user_id, job_title, job_employer, job_description, graduation_date, start_date, why, currentTime))
      self.conn.commit()
...
class ViewJobApplicationFrame(tk.Frame):
  ...
  def on_show_frame(self, event):
    ...
    database = self.cursor.fetchall()
################################
    self.cursor.execute("SELECT * FROM APPLICATIONS LEFT JOIN JOB_DATA ON APPLICATIONS.JOB_ID = JOB_DATA.JOB_ID WHERE JOB_DATA.JOB_ID IS NULL AND USER_ID = ?", (loginUsername))
    message = self.cursor.fetchone()
    print(message)
    if message is not None:
      messagebox.showwarning("Notification", "A job that you applied for has been deleted")
######################################
    try:
      self.emptyJobList_label.grid_remove()
      self.back_button.grid_remove()
...
class FriendFrame(tk.Frame):
    ...
    def search(self):
        ...
        self.search_textbox = tk.Entry(self.search_frame, width=50)
        self.search_textbox.grid(row=0, column=0, padx=5, pady=5)
        # Create and add the search button widget
        search_button = tk.Button(self.search_frame, text="Search", command=self.search)
        search_button.grid(row=0, column=1, padx=5, pady=5)
        backButton = tk.Button(self.search_frame, text="Back", command=lambda: self.controller.show_frame("ApplicationWindow"))
        backButton.grid(row=len(self.results)+2, column=0, padx=5, pady=5)

    def add_friends_elements(self, frame):
        # Clear the Frame widget before adding elements
        for widget in frame.winfo_children():
            widget.destroy()
        cursor = self.conn.execute(f"SELECT F.FRIEND_FIRST, F.FRIEND_LAST, P.University, P.Major, P.USERNAME FROM FRIENDS F LEFT JOIN PROFILE_DATA P ON F.FRIEND_USER = P.USERNAME WHERE F.USER_ID = '{self.userid}'")
        self.friendUsers = cursor.fetchall()
        if not self.friendUsers:
            ...
            for i, element in enumerate(self.friendUsers):
                ...
                cursor = self.conn.execute(f"SELECT USERNAME FROM PROFILE_DATA WHERE USERNAME = '{element[4]}'")
                profile_userid = cursor.fetchone()
                if profile_userid is not None:
                  profile_username = element[4]
                  button1 = tk.Button(frame, text='view profile', command=lambda: self.show_friend_profile(profile_username))
                  button1.grid(row=i, column=2, padx=5, pady=5)

    def show_friend_profile(self, profile_username):
      global selectedProfileUsername
      print(profile_username)
      selectedProfileUsername = profile_username
      self.controller.show_frame("DisplayProfileFrame")
  ...
  def inbox(self):
    ...
    if len(self.messages) == 0:
      ...
      for i in range(0, len(self.messages)):
        messageList.append(f'From: {self.messages[i][0]}, \nMessage: "{self.messages[i][1]}"')
      ...
      self.clicked = StringVar()
      self.clicked.set("Select message")
...
class DisplayProfileFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        #global loginUsername
        global selectedProfileUsername
        self.profUsername = selectedProfileUsername#loginUsername
        self.profile = {'title': '',
            'major': '',
            'university': '',
            'info': '',
        ...
        self.name_label.config(font=('DejaVu Sans', 20))
    ...
    def on_show_frame(self, event):
        ...
        #global loginUsername
        global selectedProfileUsername
        profile_data_query=("SELECT * FROM PROFILE_DATA WHERE USERNAME='"+ selectedProfileUsername +"'")#loginUsername
        ...
        if profile_data == None:
          self.name_label['text'] = f"No profile has been created for this account. Please create one!"
          self.title_label['text'] = f"Title: "
          self.major_label['text'] = f"Major: "
          self.university_label['text'] = f"University: "
          self.info_label['text'] = f"About: "
          self.experience_label['text'] = f"Experience: "
          self.education_label['text'] = f"Education: "
        else:
          ...
          for experience in self.profile['experience']:
            self.experience_line_label = tk.Label(self, text=f"- {experience}")
            self.experience_line_label.grid(row=6, column=0, padx=10, pady=10)

          for education in self.profile['education']:
            self.education_line_label = tk.Label(self, text=f"- {education}")
            self.education_line_label.grid(row=8, column=0, padx=10, pady=10)
